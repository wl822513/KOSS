...
from KOSS.KOSS2 import KOSS, KOSSConfig
...
class KOSSModel(nn.Module):
    def __init__(self, input_features, KOSS_hidden_size, KOSS_num_layers,
                 output_time_steps, output_features, d_state, expand_factor, d_conv):
        super(KOSSModel, self).__init__()
        self.output_time_steps = output_time_steps  # 96
        self.output_features = output_features  # D'

        # Mamba (KOSS) 层配置
        self.config = KOSSConfig(d_model=KOSS_hidden_size, n_layers=KOSS_num_layers, d_state=d_state, expand_factor=expand_factor, d_conv=d_conv)
    ...
    def forward(self, x):
        ...
        # 添加残差连接
        KOSS_out = KOSS_out + self.residual(KOSS_out)  # (B, 96, KOSS_hidden_size)
...
# 参数配置
paramsKOSS = {
    "KOSS_hidden_size": 16,               # Mamba 隐藏层大小
    "KOSS_num_layers": 2,                 # Mamba 层数
    "input_time_steps": 192,                 # 输入时间步数
    "output_time_steps": 192,                # 输出时间步数
    "input_features": 5,                   # 输入特征的数量
    "output_features": 2,                  # 输出特征的数量
    "num_epochs": 50,                      # 训练的总 epochs 数
    "period": 4.5,                         # 训练周期
    "batch_size": 32,                      # 批大小
