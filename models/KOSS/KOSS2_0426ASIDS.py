...
class KOSSConfig:
    ...
    d_state: int = 1  # N in paper/comments
...
class KOSSBlock(nn.Module):
    def __init__(self, config: KOSSConfig):
        ...
        self.MLP_IDS = nn.Sequential(
            nn.Linear(config.d_inner, 2*config.d_inner),
            nn.ReLU()
        )
        self.S6_proj = nn.Linear(config.d_inner, config.d_state, bias=False)

        self.S6_MLP = nn.Sequential(
            nn.Linear(config.d_inner, config.d_inner),
            nn.ReLU()
        )
    ...
    def seg_selective_scan(self, x, delta, A, C, D):
        ...
        for start in range(0, L, segment_size):
            ...
            # A_k, B_k = self.S4SS()
            # deltaA_k = A_k.unsqueeze(0).unsqueeze(0).repeat(B, seg_len, 1, 1)
            # deltaB_k = B_k
            # deltaB_kx = deltaB_k.unsqueeze(-1) * x_seg.unsqueeze(-2)  # (B, seg_len, ED, N)

            # B = self.S6_proj(x_seg)
            # deltaA_k = torch.exp(delta_seg.unsqueeze(-1) * A)  # (B, seg_len, ED, N) discrete parameter A_k
            # deltaB_k = delta_seg.unsqueeze(-1) * B.unsqueeze(2)  # (B, L, ED, N)
            # # deltaB_kx = deltaB_k * x_seg.unsqueeze(-1) + Kdy_dt.unsqueeze(-1)  # (B, seg_len, ED, N)
            # deltaB_kx = deltaB_k * x_seg.unsqueeze(-1)  # (B, seg_len, ED, N)

            h = (self.MLP(x_seg)).unsqueeze(-1)

            # h, _ = self.Attention(x_seg, x_seg, x_seg)
            # h = h.unsqueeze(-1)
