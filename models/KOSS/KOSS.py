...
class KOSSConfig:
    ...
    d_state: int = 4  # N in paper/comments
...
class KOSSBlock(nn.Module):
    def __init__(self, config: KOSSConfig):
        ...
        self.x_proj = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(config.d_inner, config.dt_rank + config.d_state + config.d_state)  # 输出层
        )
    ...
    def ssm(self, x):
        ...
        delta, C, K = torch.split(deltaCK, [self.config.dt_rank,  self.config.d_state,  self.config.d_state], dim=-1)
        delta = F.softplus(self.dt_proj(delta))  # (B, L, ED)
        K = F.softplus(K)

        # Kdy_dt = self.ffd_layer(x)
        Kdy_dt = delta * self.FDU(x, delta)
        Aprime, Bprime = self.SSOL(A, C, K)

        if self.config.pscan:
            y = self.selective_scan(x, delta, Aprime, Bprime, C, D, Kdy_dt)
        else:
...
class SSOL(nn.Module):
    ...
    def forward(self, A, C, K):
        ...
        """
        KCA = (K * C).unsqueeze(-2) * A  # 计算 KCA
        A_KCA = A - KCA  # 计算 A - KCA
        Aprime = A_KCA + A_KCA * (K * C).unsqueeze(-2)  # 计算 Aprime
