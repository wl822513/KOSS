...
class KOSSBlock(nn.Module):
    ...
    def seg_selective_scan(self, x, delta, A, C, D):
        ...
        for start in range(0, L, segment_size):
            ...
            # Compute A_k and B_k
            A_k, B_k = self.SSOL(deltaA_seg, C_seg, K)  # (B, seg_len, ED, N), (B, seg_len, ED, N)

            # Compute B_k * x + K * d(x)/dt
            B_kx = B_k * x_seg.unsqueeze(-1) + Kdy_dt.unsqueeze(-1)  # (B, seg_len, ED, N)

            # pscan update
            h = pscan(A_k, B_kx)  # (B, seg_len, ED)
